...
To assess out-of-sample predictive validity, we used 10-fold cross-validation and leave-one-out cross-validation (LOOCV) with Ridge regression predicting SI from the five per-probe risk composites (5 features). Ridge regression was chosen to guard against overfitting given the small sample and moderate feature count. To evaluate probe subset selection, we additionally performed nested cross-validation: within each LOOCV fold, exhaustive subset search identified the optimal probe combination on the training set, and this combination was then evaluated on the held-out participant. This guards against overfitting in probe selection, which can inflate in-sample subset performance.
...
Compared to direct analysis of participants' real sent messages, the arena produced stronger associations for the majority of EMA outcomes. The advantage was most pronounced for interpersonal constructs: perceived burdensomeness (arena $r = .403$ vs.\ raw messages $r = .079$), thwarted belonging ($.474$ vs.\ $.217$), and absolutist thinking ($.607$ vs.\ $.113$). Raw messages, however, showed stronger associations for explicit crisis content such as non-substance methods ($.450$ vs.\ $.073$) and hopelessness ($.399$ vs.\ $.306$)---categories where the actual words used in real messages carry direct clinical meaning that the arena's generated language may dilute.
...
\begin{figure}[H]
    ...
    \caption{Per-probe correlations with suicidal ideation ($N = 64$). * $p < .05$.}
    \label{fig:probes}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.55\textwidth]{figures/fig2_shuffle_validation.pdf}
...
Theory-driven probe engineering did not achieve its intended purpose of construct-specific prediction; the Demanding Authority probe failed entirely, and no targeted probe outperformed the unstructured arena for its intended construct.
...
The split-half reliability of .538 falls below the conventional .70 threshold for stable individual differences, suggesting that the adapters capture a mixture of stable communicative dispositions and time-varying patterns. This is not necessarily a limitation. EMA studies have repeatedly shown that suicidal ideation fluctuates substantially within individuals over short timescales \citep{kleiman2017examination}, and our own EMA data showed substantial within-person variability over the 28-day assessment period (passive SI $SD = 0.93$; active SI $SD = 0.85$). Against this backdrop, an adapter that fully captured trait-level communication style would risk missing precisely the dynamic component that distinguishes elevated risk states from baseline. Crucially, SI prediction was preserved across both temporal splits ($r = .359$ and $r = .396$), indicating that the core clinical signal is sufficiently stable for practical use while leaving room for the kind of state-level variation that EMA work has established as clinically important.
