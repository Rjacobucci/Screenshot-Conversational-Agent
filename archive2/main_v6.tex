...
We evaluated three conversational paradigms of increasing structure, each using the same per-person LoRA adapters and identical generation parameters (temperature $= 0.9$, top-$p = 0.95$, repetition penalty $= 1.3$, maximum 200 new tokens per turn). All conversations were scored using the same two-layer dictionary system described below. See Figure~\ref{fig:worked_example} for an illustration of the complete pipeline. 
...
We defined an agent's \textit{suicide mention rate} as the count of that agent's turns containing the literal stem ``suicid'' (matching \textit{suicide}, \textit{suicidal}, \textit{suicidality}, etc.; regex \texttt{\textbackslash bsuicid\textbackslash w*\textbackslash b}) divided by the number of paired conversations in which the agent participated (70 per agent). We use this narrow pattern because explicit mentions of \textit{suicide} are clinically unambiguous; broader risk-language matches are addressed by the dictionary-based composites below. Because the resulting rates were zero-inflated, we report Spearman alongside Pearson correlations.

The rate at which agents mentioned suicide was significantly associated with real-world SI (Pearson $r = .269$, $p = .032$; Spearman $\rho = .438$, $p < .001$). Agents who mentioned suicide at least once had significantly higher real SI ($M = 2.78$) than those who never mentioned it ($M = 1.22$), $t(62) = 2.98$, $p = .004$, Cohen's $d = 0.77$. Figure~\ref{fig:suicide_excerpts} presents representative excerpts in which agents spontaneously used suicide language during arena conversations, drawn from agents spanning the SI distribution.
...
\subsubsection{Binary vs.\ Continuous SI Outcome}
...
\bibitem[Ammerman et~al.(2026)]{ammerman2026dictionary}
[Blinded for review]. (2026).
